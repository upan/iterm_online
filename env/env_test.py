...
def test_replace(self):
  ...
  ##把/replace_file_t/文件下需要替换的文件替换到项目中
  for i in local_n:
    ...
    if file_path == '':
      ...
      cp_cmd = 'cp -rf ' + parent_path + '/replace_file_t/' + self + '/' + iterm_name + ' '  + file_path
      ##执行替换命令
...
def create_version_file(self):
  ##创建项目版本文件夹
  Get_term_path = parent_path + '/term_V_t/' 
  fp = os.listdir('../term_V_t')
  if self not in fp:
...
def file_create_version(iterm):
  ...
  file_r = 'cp -r ' +  parent_path  + '/git_download/' + iterm + ' '  + parent_path + '/term_V_t/' +  iterm + '/' + fp_file + '/'
...
##生成在/term_V_t/下的最新包
def file_create_online_test(iterm):
  test_replace(iterm)
  create_version_file(iterm)
  file_create_version(iterm)
  ##删除原有的项目包
  iterm_r_cmd = 'rm -rf ' + parent_path + '/Iterm/' + 'iterm_t' + '/' + iterm 
  (Re,Out) = commands.getstatusoutput(iterm_r_cmd)
  if Re == 0:
    logs.info_all('rm %s/iterm_t/%s success' %(parent_path,iterm))
  else:
    logs.error_all('rm %s/iterm_t/%s false' %(parent_path,iterm))
    sys.exit()
  fp_file = get_new_version(iterm)
  ##最新版本的包复制到/Iterm/iterm_t/下
  file_r = 'cp -r ' + parent_path + '/term_V_t/' +  iterm + '/' + fp_file + '/' +  iterm  + ' ' + parent_path + '/Iterm/iterm_t/' 
  (Re,Out) = commands.getstatusoutput(file_r)
  if Re == 0:
    logs.info_all('cp new_version to %s/Iterm/iterm_t/%s' %(parent_path,iterm))
  else:
    logs.error_all('cp new_version to %s/Iterm/iterm_t/%s is false!!!' %(parent_path,iterm))
    sys.exit()


##回退测试的上一个版本的内容在/Iterm/iterm_t下
def Rollback_test(iterm):
  ##回退测试版本
  ite_t_cmd = 'rm -rf ' + parent_path + '/Iterm/iterm_t/' + iterm
  (Re,Out) = commands.getstatusoutput(ite_t_cmd)
  if Re == 0:
    logs.info('rm %s/Iterm/iterm_t/%s success' %(parent_path,iterm))
  else:
    logs.error_all('something error happend')
  ter_v = get_new_version(iterm)
  ite_r_ver = 'rm -rf ' + parent_path + '/term_V_t/' +  iterm + '/'  + ter_v
  (Re,Out) = commands.getstatusoutput(ite_r_ver)
  ter_v = get_new_version(iterm)
  ter_c = 'cp -r ' + parent_path + '/term_V_t/' +  iterm + '/' + ter_v + '/' +  iterm  + ' ' + parent_path + '/Iterm/iterm_t/'
  (Re,Out) = commands.getstatusoutput(ter_c)
  if Re == 0:
    logs.info_all('Now already back to %s in %s/Iterm/iterm_t/%s' %(ter_v,parent_path,iterm))

   
##获取最新版本名
def get_new_version(iterm):
  dir_v = parent_path + '/term_V_t/' + iterm
